-- Migration 012: Fix leaderboard matches_played counters
-- Updates the update_game_leaderboard function to increment regular_matches_played and ranked_tournaments_played
-- This ensures all match participation is properly tracked
...
-- Backfill existing leaderboard records to match wins+losses with matches_played counters
UPDATE game_leaderboard
SET regular_matches_played = regular_wins + regular_losses
WHERE regular_matches_played = 0 AND (regular_wins > 0 OR regular_losses > 0);

UPDATE game_leaderboard
SET ranked_tournaments_played = ranked_wins + ranked_losses
WHERE ranked_tournaments_played = 0 AND (ranked_wins > 0 OR ranked_losses > 0);
