...
func setupRouter(db *sql.DB, minioClient *storage.MinIOClient, cfg *config.Config, nrApp *newrelic.Application, betaGroups map[int64]bool) *mux.Router {
	...
	if arenaHandler != nil {
		...
		api.HandleFunc("/arena/matches/by-message", arenaHandler.HandleGetMatchByMessage).Methods("GET")
	...
	if miniAppHandler != nil {
		...
		if arenaHandler != nil {
			...
			protected.HandleFunc("/arena/matches/active", arenaHandler.HandleGetUserActiveMatch).Methods("GET", "OPTIONS")
			...
			protected.HandleFunc("/arena/match/{id}/replay", arenaHandler.HandleGetBattleReplay).Methods("GET", "OPTIONS")
